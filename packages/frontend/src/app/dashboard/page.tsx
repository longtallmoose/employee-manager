        ...
        <div className="p-8 max-w-7xl mx-auto w-full">
          <div className="flex justify-between items-end mb-10">
            <div>
              ...
              <p className="text-slate-500 font-medium mt-1">Personnel Directory & Compliance</p>
            </div>
            <button onClick={() => setIsAddingNew(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-4 rounded-2xl font-bold shadow-xl flex items-center gap-3 transition-all active:scale-95">
              ...
              <div className="flex-1 p-8 space-y-6 overflow-y-auto">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <input className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold" placeholder="First Name" value={newEmployee.firstName} onChange={(e) => setNewEmployee({...newEmployee, firstName: e.target.value})} />
                    <input className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold" placeholder="Last Name" value={newEmployee.lastName} onChange={(e) => setNewEmployee({...newEmployee, lastName: e.target.value})} />
                  </div>
                  <input className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold" placeholder="National Insurance (NI)" value={newEmployee.niNumber} onChange={(e) => setNewEmployee({...newEmployee, niNumber: e.target.value})} />
                </div>
                <div className="space-y-4">
                  <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Home Address</p>
                  <input className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold" placeholder="Address Line 1" value={newEmployee.addressLine1} onChange={(e) => setNewEmployee({...newEmployee, addressLine1: e.target.value})} />
                  <input className="w-full p-4 bg-slate-50 border rounded-2xl outline-none font-bold" placeholder="Postcode" value={newEmployee.postcode} onChange={(e) => setNewEmployee({...newEmployee, postcode: e.target.value})} />
                </div>
                <div className="space-y-4">
                  <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Next of Kin</p>
                  <div className="grid grid-cols-2 gap-4">
                    <input className="p-4 bg-slate-50 border rounded-2xl outline-none font-bold" placeholder="Contact Name" value={newEmployee.emergencyName} onChange={(e) => setNewEmployee({...newEmployee, emergencyName: e.target.value})} />
                  ...
                  <div className="space-y-8">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 shadow-sm">
                        ...
                        <input disabled={!isEditing} className="bg-transparent font-black text-lg outline-none w-full" value={selectedEmployee.records[0]?.department} onChange={(e) => {
                          const recs = [...selectedEmployee.records];
                  ...
                  <div className="space-y-8">
                    <div className="p-6 bg-slate-50 rounded-[2.5rem] border space-y-6 shadow-sm">
                      <div className="space-y-3">
                        ...
                        <input disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border outline-none font-bold shadow-sm" value={selectedEmployee.addressLine1 || ''} onChange={(e) => setSelectedEmployee({...selectedEmployee, addressLine1: e.target.value})} />
                        <input disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border outline-none font-bold shadow-sm" value={selectedEmployee.postcode || ''} onChange={(e) => setSelectedEmployee({...selectedEmployee, postcode: e.target.value})} />
                      </div>
                    </div>
                    <div className="p-6 bg-slate-50 rounded-[2.5rem] border space-y-6 shadow-sm">
                      ...
                      <input disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border outline-none font-bold shadow-sm" placeholder="Contact Name" value={selectedEmployee.emergencyName || ''} onChange={(e) => setSelectedEmployee({...selectedEmployee, emergencyName: e.target.value})} />
                      <input disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border outline-none font-bold shadow-sm" placeholder="Phone Number" value={selectedEmployee.emergencyPhone || ''} onChange={(e) => setSelectedEmployee({...selectedEmployee, emergencyPhone: e.target.value})} />
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-8">
                    <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white space-y-6 shadow-2xl relative">
                      <div className="flex justify-between items-center relative z-10">
                        <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">UK Compliance</h4>
