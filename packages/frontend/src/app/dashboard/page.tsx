    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden text-slate-900 font-sans">
      ...
      <main className="flex-1 flex flex-col overflow-y-auto relative">
        ...
        {isPanelOpen && selectedEmployee && (
          <div className="fixed inset-0 z-50 overflow-hidden flex justify-end bg-slate-900/50 backdrop-blur-sm" onClick={() => setIsPanelOpen(false)}>
            <div className="max-w-xl w-full bg-white h-full flex flex-col p-8 space-y-8 animate-in slide-in-from-right duration-500 shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="p-8 pb-4 flex flex-col gap-6">
                <div className="flex justify-between items-start">
                  <div className="flex items-center gap-5">
                    <div className="w-20 h-20 rounded-[2rem] bg-slate-900 text-white flex items-center justify-center text-3xl font-black shadow-2xl shadow-slate-200">{selectedEmployee.firstName[0]}</div>
                    <div>
                  ...
                  <div className="space-y-8">
                    <div className="p-6 bg-slate-50 rounded-3xl border space-y-6">
                      <div className="space-y-2">
                        ...
                        <input disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border font-bold" value={selectedEmployee.records[0]?.jobTitle} onChange={e => {
                      ...
                      <div className="space-y-2">
                        ...
                        <select disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border font-bold" value={selectedEmployee.records[0]?.department} onChange={e => {
                    ...
                    {/* RESTORED TIMELINE COMPONENT */}
                    <div className="space-y-4">
                      <h4 className="font-black text-xs uppercase tracking-widest flex items-center gap-2 text-slate-900"><History size={18}/> Service History</h4>
                      <div className="space-y-6 border-l-4 border-slate-100 ml-2 pl-8 relative">
                        {selectedEmployee.records.map((rec: any, idx: number) => (
                          <div key={rec.id} className="relative py-2">
                            ...
                            {rec.changeReason && <p className="text-xs text-slate-500 italic mt-1">{rec.changeReason}</p>}
                          </div>
                        ))}
                      </div>
                    </div>
              ...
              <div className="p-8 border-t bg-slate-50 grid grid-cols-2 gap-4">
                {isEditing ? (
                  <><button onClick={() => setIsEditing(false)} className="py-4 bg-white border border-slate-200 rounded-2xl font-black uppercase text-xs tracking-widest">Discard</button><button onClick={handleUpdate} className="py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl flex items-center justify-center gap-2"><Save size={16}/> Save Changes</button></>
                ) : (
