    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden text-slate-900 font-sans">
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col z-20 shadow-2xl">
        ...
        <nav className="flex-1 mt-6 px-3 space-y-1">
          <button onClick={() => setActiveTab('Employees')} className="w-full flex items-center gap-3 px-3 py-3 rounded-xl bg-blue-600 text-white"><Users size={20} /> <span className="font-semibold text-sm">Employees</span></button>
        </nav>
      </aside>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto relative">
        ...
        {/* ONBOARD SCREEN */}
        {isAddingNew && (
          <div className="fixed inset-0 z-50 overflow-hidden flex justify-end bg-slate-900/50 backdrop-blur-sm">
            <div className="max-w-xl w-full bg-white h-full flex flex-col p-8 space-y-6 overflow-y-auto animate-in slide-in-from-right duration-500 shadow-2xl">
              <div className="flex justify-between items-center"><h3 className="text-2xl font-black">New Hire</h3><button onClick={() => setIsAddingNew(false)}><X size={24}/></button></div>
              ...
              <button onClick={handleCreate} className="w-full py-5 bg-blue-600 text-white rounded-3xl font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">Onboard Personnel</button>
            </div>
          </div>
        )}
        ...
        {isPanelOpen && selectedEmployee && (
          <div className="fixed inset-0 z-50 overflow-hidden flex justify-end bg-slate-900/50 backdrop-blur-sm">
            <div className="max-w-xl w-full bg-white h-full flex flex-col p-8 space-y-8 animate-in slide-in-from-right duration-500 shadow-2xl">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-5"><div className="w-20 h-20 rounded-[2.5rem] bg-slate-900 text-white flex items-center justify-center text-3xl font-black">{selectedEmployee.firstName[0]}</div><div><h3 className="text-3xl font-black">{selectedEmployee.firstName} {selectedEmployee.lastName}</h3><p className="text-blue-600 font-bold text-xs uppercase tracking-widest">{selectedEmployee.records[0]?.jobTitle}</p></div></div>
              ...
              <div className="flex-1 overflow-y-auto space-y-8">
                {profileTab === 'Overview' && (
                  <div className="space-y-6">
                    <div className="p-6 bg-slate-50 rounded-3xl border space-y-6">
                      <div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase">Position</p><input disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border font-bold" value={selectedEmployee.records[0].jobTitle} onChange={e => {
                        const r = [...selectedEmployee.records]; r[0].jobTitle = e.target.value; setSelectedEmployee({...selectedEmployee, records: r})
                      }} /></div>
                      <div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase">Department</p><input disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border font-bold" value={selectedEmployee.records[0].department} onChange={e => {
                ...
                {profileTab === 'Personal' && (
                  <div className="p-6 bg-slate-50 rounded-3xl border space-y-6">
                    <div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Residence</p><input disabled={!isEditing} className="w-full bg-white p-4 rounded-2xl border font-bold" value={selectedEmployee.addressLine1 || ''} onChange={e => setSelectedEmployee({...selectedEmployee, addressLine1: e.target.value})} /></div>
                  </div>
                )}
                {profileTab === 'Payroll' && (
                  <div className="p-8 bg-slate-900 rounded-[2.5rem] text-white space-y-6">
                    <div className="flex justify-between items-center"><h4 className="text-[10px] font-black uppercase text-slate-400">Payroll Compliance</h4><button onClick={() => setShowSensitive(!showSensitive)} className="text-[10px] font-bold bg-white/20 px-4 py-2 rounded-full">{showSensitive ? 'HIDE' : 'SHOW'}</button></div>
                    <div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase">NI Number</p><input disabled={!isEditing} type={showSensitive || isEditing ? "text" : "password"} className="bg-transparent text-2xl font-mono font-black outline-none w-full" value={selectedEmployee.niNumber || ''} onChange={e => setSelectedEmployee({...selectedEmployee, niNumber: e.target.value})} /></div>
                    <div className="pt-6 border-t border-white/10 flex justify-between"><div><p className="text-[10px] font-bold text-slate-400">Annual Salary</p><input disabled={!isEditing} className="bg-transparent text-xl font-black outline-none" value={selectedEmployee.records[0].payAmount} onChange={e => {
